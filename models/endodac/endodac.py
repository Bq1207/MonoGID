...
from .illumination_semantic_attention import IlluminationSemanticAttention
from .bifpn import ChannelPreservingEnhancedBiFPN

class DPTHead(nn.Module):
    def __init__(self, in_channels, features=128, use_bn=False, out_channels=[96, 192, 384, 768],
                 use_clstoken=False, use_isa=False, use_bifpn=False, bifpn_num_blocks=2,
                 use_geo_injection=False, geo_type='gradient', geo_fusion_type='gated'):
        ...
        if self.use_bifpn:
            self.fpn = ChannelPreservingEnhancedBiFPN(
                out_channels,
                unified_channels=256,
                num_bifpn_blocks=bifpn_num_blocks,
                use_geo_injection=use_geo_injection,
                geo_type=geo_type,
                geo_fusion_type=geo_fusion_type
            )

        # 光照-语义注意力模块（在Conv Neck输出后应用）
        if self.use_isa:
            # 原始独立ISA模块
            self.isa_1 = IlluminationSemanticAttention(features, se_reduction=16, semantic_reduction=4)
            self.isa_2 = IlluminationSemanticAttention(features, se_reduction=16, semantic_reduction=4)
            self.isa_3 = IlluminationSemanticAttention(features, se_reduction=16, semantic_reduction=4)
            self.isa_4 = IlluminationSemanticAttention(features, se_reduction=16, semantic_reduction=4)
    ...
    def forward(self, out_features, patch_h, patch_w):
        ...
        d_coarse = None
        if self.use_bifpn:
            out, d_coarse = self.fpn(out)
        
        layer_1, layer_2, layer_3, layer_4 = out
        
        # Conv Neck: 将多尺度特征转换为统一特征维度
        ...
        if self.use_isa:
            # 原始独立ISA
            layer_1_rn = self.isa_1(layer_1_rn)
            layer_2_rn = self.isa_2(layer_2_rn)
            layer_3_rn = self.isa_3(layer_3_rn)
            layer_4_rn = self.isa_4(layer_4_rn)
        ...
        # 补充 d_coarse (如果 FPN 返回了)
        if d_coarse is not None:
            outputs["d_coarse"] = d_coarse
...
class endodac(nn.Module):
    ...
    def __init__(self,
                 ...
                 use_bifpn=False,
                 bifpn_num_blocks=2,
                 use_geo_injection=False,
                 geo_type='gradient',
                 geo_fusion_type='gated'):
        ...
        self.depth_head = DPTHead(
            self.embedding_dim, self.depth_head_feature, use_bn,
            out_channels=self.depth_head_out_channel,
            use_clstoken=use_cls_token, use_isa=use_isa, use_bifpn=use_bifpn,
            bifpn_num_blocks=bifpn_num_blocks,
            use_geo_injection=use_geo_injection, geo_type=geo_type, geo_fusion_type=geo_fusion_type
        )
