...
from models.speed_optimizations.lora_fusion import fuse_lora_weights
from models.speed_optimizations.conv_bn_fusion import fuse_conv_bn_weights
...
def evaluate(opt):
    ...
    if opt.ext_disp_to_eval is None:
        ...
        if opt.model_type == 'endodac':
            depther = endodac.endodac(
                ...
                use_isa=opt.use_isa,
                use_bifpn=opt.use_bifpn,
                bifpn_num_blocks=opt.bifpn_num_blocks,
                use_geo_injection=opt.use_geo_injection,
                geo_type=opt.geo_type,
                geo_fusion_type=opt.geo_fusion_type)
            model_dict = depther.state_dict()
            depther.load_state_dict({k: v for k, v in depther_dict.items() if k in model_dict},strict=False)
            depther.cuda()
            depther.eval()
            # 融合LoRA权重以加速推理（创新点1）
            if opt.use_lora_fusion:
                fuse_lora_weights(depther)
            # 融合Conv-BN权重以加速推理（卷积层融合）
            if opt.use_conv_bn_fusion:
                fuse_conv_bn_weights(depther)
        elif opt.model_type == 'afsfm':
    ...
    with torch.no_grad():
        for i, data in tqdm(enumerate(dataloader)):
            ...
            pred_depth_low_res = 1 / pred_disp  
            pred_depth = cv2.resize(pred_depth_low_res, (gt_width, gt_height))
    ...
    for i in range(len(mean_errors)):
        ...
        cls.append(cl[1])
    cls = np.array(cls)
    print("\n     " + ("{:>13}    | " * 7).format("abs_rel", "sq_rel", "rmse", "rmse_log", "a1", "a2", "a3"))
